...
> *[..о вашем проекте узнают заинтересованные люди..](https://github.com/TezerPlatform/TezerPaymentSystem?tab=readme-ov-file#выгода-сторон-heavy_dollar_sign)*

При подключении системы оплаты, мы рассказываем во всех социальных сетях о вашем проекте, и наша аудитория может заинтересоваться вашим проектом и купить продукт. *Если возникут вопросы при подключении системы оплаты, свяжитесь с [Модератором проекта](https://t.me/tezermoder).*

Оплата происходит в [3 действия](https://github.com/TezerPlatform/TezerPaymentSystem?tab=readme-ov-file#оплата-в-3-действия-three):
+ Сначало пользователь создаёт ссылку для оплаты, где в ссылке нужно указать: 
  ...
  + И выбрать [контракт](https://github.com/TezerPlatform/TezerPaymentSystem?tab=readme-ov-file#о-контрактах-bank) для передачи токенов TZR
...
Каждая сторона становится популярнее. Как? При подключении каких-либо проектов в систему оплаты Tezer (TPS) мы рекламируем Ваш проект в наших социальных сетях, а также создаём пост в самом боте в разделе `Tezer Lents | Projects`. Аудитория Tezer Platform узнаёт о Вашем проекте, а ваша аудитория проекта узнаёт о нашей системе оплаты. Так же подберём индивидуальные условия для каждого партнёра.
По вопросам обращаться к [Модератору проекта](https://t.me/tezermoder "Официальный модератор проекта Tezer Platform")
...
1. Установка через `git` в главной папке проекта, в терминале:
...
• `amount_rox` - количество токенов, которое пользователь должен оплатить. *В моём случае это количество пользователь сам выбирает. Указывается выше 6.2 до 1,000,000.0 TZR*

• `use_contract` - Название контракта строчными буквами. Доступные контракты смотрите тут (В Github, в Telegram канале и по [ссылке](https://telegra.ph/Kontrakty-TezerPlatform-06-25)). По мере обновления мы оповещаем в Telegram канале и здесь происходят обновления.
...
+ **Данные, такие как история платежей, номера карт, пароли и другие конфеденциальные данные мы не просим вводить и не сохраняем. Если возникли проблемы с оплатой или попались на мошенника - сообщите [модератору](https://t.me/tezermoder "Официальный менеджер проекта Tezer Platform")**
...
Создать контракт может каждый желающий имеющий базу программирования. Подробнее про создание контрактов обратитесь к **[Модератору](https://t.me/tezermoder)**
